...
The 6502 is still in production by Western Design Centre. They build the W65C02S CPU which is code, but not fully pin compatible to the original 6502. So you have to make some minor changes to make it fully work with the Junior Computer ][ PCB.

- The W65C02S uses pin 1 as the Vector Pull output signal, whereas the 6502 uses this as a second VSS pin, which is tied to Ground on the JC2. 
  **To solve the problem, you can remove pin 1 on your 6502 CPU IC socket or cut the trace on the component side of the PCB that connects pin 1 of the CPU socket to Ground.**
