...
; ... added commands PORT, LOCATE, and more ...
; ... restructure of the ZP usage ...

; Version 2.25

; ... added I2CStop, Line, Color, Screen and Pixel commands

; Junior Computer 2 Monitor Routines and Constants

PROC_SETSTDOUTID	= $E018
PROC_SETSTDOUT		= $E01B
PROC_SETSTDINID		= $E02F
PROC_DEVOPEN		= $E1AA

PROC_I2C_START		= $E326
PROC_I2C_STOP		= $E335
PROC_I2C_READ_DEV	= $E362
PROC_I2C_WRITE_DEV	= $E366
PROC_I2C_SEND		= $E367
PROC_I2C_RCV		= $E39F

PROC_DELAY		= $E14D

VIA_PORTB     		= $00  		; Port B input/output register 
VIA_PORTA     		= $01  		; Port A input/output register
VIA_DDRB   		= $02		; Port B data direction register 
VIA_DDRA   		= $03		; Port A data direction register
VIA_T2CL   		= $08		; Timer 2 counter low byte register
VIA_T2CH   		= $09		; Timer 2 counter high byte register
VIA_IFR    		= $0D		; Interrupt flag register
VIA_IER    		= $0E		; Interrupt enable register

IOBASE			= $14		; IOBase Vector
...
VEC_DEVCMD		= $1A6C			; current opened device command vector (Junior Computer 2)

VEC_IN			= $1A6E     		; input vector  (Junior Computer 2)
VEC_OUT			= $1A70  	  	; output vector (Junior Computer 2)
VEC_CMD			= $1A72			; command vector (Junior Computer 2)
...
;               = $D9           ; VDP usage
;
;               = $DA           ; *used by Junior 2 system TICKCNT
;               = $DB           ; *used by Junior 2 system
...
;		= $ED		; *used by Junior 2 system
;		= $EE		; *used by Junior 2 system
...
TK_I2CSTOP	= TK_PORTOUT+1		; I2CSTOP token ###
TK_PIXEL	= TK_I2CSTOP+1		; PIXEL token ###
TK_LINE		= TK_PIXEL+1		; LINE token ###
TK_SCREEN       = TK_LINE+1		; SCREEN token ###
TK_RECT		= TK_SCREEN+1		; RECT token ###
TK_COLOR	= TK_RECT+1		; COLOR token ###
TK_DELAY	= TK_COLOR+1		; DELAY token
TK_BAR		= TK_DELAY+1		; BAR token
TK_RES4		= TK_BAR+1		; RESERVED ###
...
; These two byte are the destination address for XModem load. Remove for ROM version !!!
...
; Get Four Byte Parameters ****************
	
LAB_GET4BYTEPARMS
        JSR	LAB_GTBY		; get first byte parameter into X
        STX     ADRSL                   ; and store it in temp var x1
        JSR	LAB_1C01		; check for "," else syntax error
	JSR	LAB_GTBY		; get second byte parameter into X
	STX     ADRSH                   ; and store it in temp var x2
	JSR	LAB_1C01		; check for "," else syntax error
	JSR	LAB_GTBY		; get second byte parameter into X
	STX     ADREL                   ; and store it in temp var x3
	JSR	LAB_1C01		; check for "," else syntax error
	JSR	LAB_GTBY		; get second byte parameter into X
	STX     ADREH                   ; and store it in temp var x4
	LDX     ADRSL                   ; restore x1
	LDY     ADRSH                   ; restore x2
	RTS
...
LAB_I2CSTOP
        JMP	PROC_I2C_STOP		; send stop condition
...
LAB_SCREEN
	JSR	LAB_GTBY		; scan for byte value
        LDA     #CMD_SET_MODE
	JMP 	(VEC_CMD)		; Call Junior Computer standard SET_MODE routine
...
LAB_PIXEL
	BEQ	LAB_SYNTAX_ERR2		; if no following token, exit and throw syntax error
	JSR	LAB_GET2BYTEPARMS	; get two parameters
	LDA     #CMD_PUT_PIXEL
	JMP 	(VEC_CMD)		; Call Junior Computer standard SET_MODE routine
...
LAB_LINE
	BEQ	LAB_SYNTAX_ERR2		; if no following token, exit and throw syntax error
        JSR     LAB_GET4BYTEPARMS       ; get four parameters
	LDA     #CMD_MOVE_TO
	JSR 	LAB_CALL_CMD		; Call Junior Computer standard MOVE_TO routine
	LDX     ADREL
	LDY     ADREH
	LDA     #CMD_DRAW_LINE
	JMP 	(VEC_CMD)		; Call Junior Computer standard DRAW_LINE routine
...
LAB_RECT
	...
	LDY     ADREH
	LDA     #CMD_DRAW_RECT
	JMP 	(VEC_CMD)		; Call Junior Computer standard DRAW_RECT routine
...
LAB_SYNTAX_ERR2
        JMP	LAB_SNER		; throw syntax error
        

; *****************************************

LAB_CALL_CMD
       JMP 	(VEC_CMD)		; Call Junior Computer standard command routine
...
V_INPT
	JMP     CGET     		; non halting scan input device
V_OUTP
...
LAB_SMSG
	.byte	" Basic Bytes free",$0D,$0A,$0A,$00
...
LAB_CTBL
	...
	.word   LAB_I2CSTOP-1           ; I2CSTOP       new command (Junior Computer 2)
	...
	.word	LAB_BAR-1		; BAR		new command (Junior Computer 2)
...
TAB_ASCI
LBB_I2CSTOP
        .byte   "2CSTOP",TK_I2CSTOP     ; I2CSTOP
LBB_IF
	.byte	"F",TK_IF		; IF
LBB_INNUM
	.byte	"N#",TK_INNUM		; IN#
